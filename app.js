...
function memberCardScreenshot(project) {
  ...
  return thumbnailUrl
    ? `<a class="member-card-screenshot-link" href="${escapeHtml(thumbnailUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Open full screenshot in new tab"><span class="member-card-screenshot-label">Screenshot</span><img class="member-card-screenshot" src="${escapeHtml(thumbnailUrl)}" alt="Submitted build screenshot" loading="lazy" decoding="async" /></a>`
    : "";
}
...
function projectCard(p) { return `<article class="card"><h3>${p.title}</h3><div class="title-rule"></div><p>${p.summary}</p><p class="muted"><strong>Creator:</strong> ${p.creatorName}</p><div class="badges">${p.categories.map((c) => `<span class="badge">${c}</span>`).join("")}<span class="badge status">Stage: ${escapeHtml(p.status || "Idea")}</span>${p.featured ? '<span class="badge">House Favorite</span>' : ""}</div>${memberCardScreenshot(p)}<details class="member-pour-details"><summary>View details</summary>${memberDetailPanel(p)}<button class="button" type="button" onclick="this.closest('details').open=false">Close details</button></details></article>`; }
...
async function signMemberScreenshotUrls(projects){const paths=[...new Set(projects.map((p)=>String(p.screenshotPath||"").trim()).filter((path)=>path&&!isAbsoluteHttpUrl(path)))];if(!paths.length)return;console.log("[Prompt & Pour] sign_member_screenshots paths:",paths);if(!state.memberToken){console.warn("[Prompt & Pour] Member token missing; screenshot URLs cannot be signed for member view.");return;}const u=getMemberFunctionUrl();if(!u)return;const r=await fetch(u,{method:"POST",headers:{"Content-Type":"application/json","x-prompt-pour-member-token":state.memberToken},body:JSON.stringify({action:"sign_member_screenshots",paths})});if(!r.ok){console.warn("[Prompt & Pour] Unable to sign screenshot URLs for member view.",r.status);return;}const b=await r.json();console.log("[Prompt & Pour] sign_member_screenshots response keys:",Object.keys(b?.signed||{}));for(const project of projects){if(project.screenshotPath&&isAbsoluteHttpUrl(b?.signed?.[project.screenshotPath])){project.screenshotSignedUrl=b.signed[project.screenshotPath];project.screenshotUrl=project.screenshotSignedUrl;}}const signedCount=projects.filter((project)=>isAbsoluteHttpUrl(project.screenshotSignedUrl)).length;console.log("[Prompt & Pour] screenshotSignedUrl populated after merge:",signedCount>0,`(${signedCount}/${projects.length})`);}
