...
function topNav() {
  ...
  return `<header class="topbar"><div class="topbar-brand-row"><div class="brand-copy"><p class="brand-line"><span class="brand-primary">Prompt & Pour:</span><span class="brand-secondary">A Private Exchange</span></p><p class="brand-subhead">A back room for colleagues exploring A.I. before the playbook exists.</p></div></div></header><div class="topbar-menu"><div class="topbar-nav-row"><nav class="nav">${navButton("Home", "dashboard")}${navButton("House Pours", "gallery")}${navButton("Share a Build", "share")}${state.role === "admin" || state.adminToken ? navButton("Admin", "admin") : ""}${navButton("House Rules", "rules")}<button onclick="logout()">Exit</button></nav></div></div>`;
}
