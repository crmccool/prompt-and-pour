...
async function refreshAdminLists() { state.adminLoading = true; render(); try { const p = await callAdminAction("list_pending"); const a = await callAdminAction("list_approved"); const ar = await callAdminAction("list_archived"); state.adminPending = (p.rows || []).map(mapRowToProject); state.adminApproved = (a.rows || []).map(mapRowToProject); state.adminArchived = (ar.rows || []).map(mapRowToProject); await signAdminScreenshotUrls(state.adminPending); await signAdminScreenshotUrls(state.adminApproved); await signAdminScreenshotUrls(state.adminArchived); await refreshAdminEvents(); state.adminError = ""; } catch (e) { state.adminError = e.message; } state.adminLoading = false; render(); }
async function refreshAdminEvents(){const ev=await callAdminAction("list_events");state.adminEvents=ev.rows||[];}
async function signAdminScreenshotUrls(projects){const paths=[...new Set(projects.map((p)=>String(p.screenshotPath||"").trim()).filter((path)=>path&&!isAbsoluteHttpUrl(path)))];if(!paths.length||!state.adminToken)return;const r=await callAdminAction("sign_admin_screenshots","",{paths});const signed=r?.signed||{};for(const project of projects){if(project.screenshotPath&&isAbsoluteHttpUrl(signed[project.screenshotPath])){project.screenshotSignedUrl=signed[project.screenshotPath];project.screenshotUrl=project.screenshotSignedUrl;}}}
...
function render() { const app = document.getElementById("app"); if (!app) return; if (!state.loggedIn) app.innerHTML = loginPage(); else { const pages = { dashboard: dashboardPage, gallery: galleryPage, sessions:sessionsPage, ledger:ledgerPage, share: sharePage, "share-success": shareSuccessPage, admin: adminPage, rules: rulesPage, project: projectDetailPage }; app.innerHTML = `<div class="layout">${topNav()}<main class="main">${(pages[state.route] || dashboardPage)()}</main>${dataStatusPill()}</div>`; } }
