...
function setRoute(route, projectId = null) { const nextRoute = route === "admin" && state.role !== "admin" ? "dashboard" : route; if (state.route === "dashboard" && nextRoute !== "dashboard") state.dashboardNotice = ""; state.route = nextRoute; state.selectedProjectId = projectId; render(); }
...
function adminPage() { const filtered = state.projects.filter(matchesFilters); const pending = filtered.filter((p) => !p.approved && !p.archived); const approved = filtered.filter((p) => p.approved && !p.archived); const featured = filtered.filter((p) => p.featured && !p.archived); const archived = filtered.filter((p) => p.archived); return `<section class="panel hero"><h1 class="section-title">Admin Dashboard</h1><section class="admin-moderation-panel" aria-label="Admin / Moderation"><h2 class="section-title">Admin / Moderation</h2><p class="muted"><strong>Admin moderation is not yet connected.</strong> New submissions are stored in Supabase with <code>approved = false</code> and must be approved manually in the Supabase Table Editor for now.</p><ol class="admin-steps"><li>Go to Supabase</li><li>Open <code>public.prompt_pour_pours</code></li><li>Review submitted rows</li><li>Change <code>approved</code> from <code>false</code> to <code>true</code> to publish</li><li>Optional: change <code>featured</code> from <code>false</code> to <code>true</code> to show in House Favorites</li></ol></section>${filterControls()}<h2 class="section-title">Pending</h2><div class="grid">${pending.map((p) => projectCard(p, true)).join("") || "<p>None</p>"}</div><h2 class="section-title">Approved</h2><div class="grid">${approved.map((p) => projectCard(p, true)).join("") || "<p>None</p>"}</div><h2 class="section-title">Featured</h2><div class="grid">${featured.map((p) => projectCard(p, true)).join("") || "<p>None</p>"}</div><h2 class="section-title">Archived</h2><div class="grid">${archived.map((p) => projectCard(p, true)).join("") || "<p>None</p>"}</div></section>`; }
...
function logout() { state.loggedIn = false; state.role = "member"; state.notice = ""; state.dashboardNotice = ""; setRoute("login"); }
function dismissDashboardNotice() { state.dashboardNotice = ""; render(); }
...
Object.assign(window, { setRoute, login, logout, setCategoryFilter, setStatusFilter, submitPour, dismissDashboardNotice });
