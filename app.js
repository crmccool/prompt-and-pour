...
function topNav() { if (!state.loggedIn) return ""; return `<header class="topbar"><div class="topbar-row"><div class="brand"><img class="brand-mascot" src="deco-bartender.png" alt="Art Deco robot bartender mascot" loading="lazy" decoding="async" /><div class="brand-copy">Prompt & Pour <small>Private Exchange</small></div></div><nav class="nav">${navButton("Home", "dashboard")}${navButton("House Pours", "gallery")}${navButton("Share a Build", "share")}${state.role === "admin" || state.adminToken ? navButton("Admin", "admin") : ""}${navButton("House Rules", "rules")}<button onclick="logout()">Exit</button></nav></div></header>`; }
