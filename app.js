...
async function refreshAdminLists() { state.adminLoading = true; render(); try { const p = await callAdminAction("list_pending"); const a = await callAdminAction("list_approved"); const ar = await callAdminAction("list_archived"); state.adminPending = (p.rows || []).map(mapRowToProject); state.adminApproved = (a.rows || []).map(mapRowToProject); state.adminArchived = (ar.rows || []).map(mapRowToProject); await refreshAdminEvents(); state.adminError = ""; } catch (e) { state.adminError = e.message; } state.adminLoading = false; render(); }
...
function render() { const app = document.getElementById("app"); if (!app) return; if (!state.loggedIn) app.innerHTML = loginPage(); else { const pages = { dashboard: dashboardPage, gallery: galleryPage, sessions:sessionsPage, ledger:ledgerPage, share: sharePage, admin: adminPage, rules: rulesPage, project: projectDetailPage }; app.innerHTML = `<div class="layout">${topNav()}<main class="main">${(pages[state.route] || dashboardPage)()}</main>${dataStatusPill()}</div>`; } }
