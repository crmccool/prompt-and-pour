...
const hasSupabaseConfig = Boolean(supabaseConfig?.url && supabaseConfig?.anonKey && window.supabase?.createClient);
const supabase = hasSupabaseConfig ? window.supabase.createClient(supabaseConfig.url, supabaseConfig.anonKey) : null;
...
function setRoute(route, projectId = null) {
  state.route = route === "admin" && state.role !== "admin" ? "dashboard" : route;
...
async function submitPour(event) {
  ...
  state.notice = "Mock mode: your pour was captured locally only.";
  form.reset();
  render();
}
