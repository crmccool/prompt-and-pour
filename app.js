...
function mapRowToProject(row) { return { id: row.id, title: row.title || "Untitled Pour", summary: row.summary || "", creatorName: row.creator_name || "Anonymous", contactEmail: row.creator_email || row.contact_email || "", categories: Array.isArray(row.categories) && row.categories.length ? row.categories : ["Other / Not sure"], status: row.status || "Idea", toolsUsed: Array.isArray(row.tools_used) ? row.tools_used.join(", ") : row.tools_used || "", problemSolved: row.problem_statement || "", howAiHelped: row.ai_use || "", lessonsLearned: row.lessons_learned || "", helpWanted: row.help_wanted || "", reusableBits: row.reusable_bits || "", links: Array.isArray(row.links) ? row.links : [], screenshotUrl: row.screenshot_url || "", screenshotPlaceholder: "Screenshot placeholder", reusePermission: row.reuse_permission || "", createdDate: (row.created_at || "").slice(0, 10), updatedDate: (row.updated_at || row.created_at || "").slice(0, 10), approved: !!row.approved, featured: !!row.featured, archived: !!row.archived }; }
...
function adminCard(p, buttons, showExpandedDetails = false) {
  ...
  return `<article class="card"><h3>${p.title}</h3>${adminField("Creator", p.creatorName)}${adminField("Contact Email", p.contactEmail)}${adminListField("Categories", p.categories)}${adminField("Status", p.status)}${adminField("Summary", p.summary)}${adminField("Created Date", p.createdDate)}${showExpandedDetails && fullDetails ? `<details><summary>View full submission</summary>${fullDetails}</details>` : fullDetails}<div class="admin-actions">${buttons}<button class='button' onclick="startAdminEdit('${p.id}')">Edit</button></div></article>`;
}
